...
use walletd_hd_key::{ExtendedPrivateKey, ExtendedPublicKey, HDKey, HDNetworkType, Seed, HDPath, HDPurpose, Error};
...
fn test_new_func_bip32_first_account() -> Result<(), Error> {
    ...
    let keys = HDKey::new(
        ...
        HDNetworkType::MainNet, format!("m/{}/{}'/0'", dt, Coin::from(Symbol::BTC).id())
    )?;
...
fn test_new_func_first_BIP32_address() -> Result<(), Error> {
    ...
    let keys = HDKey::new(
        ...
        HDNetworkType::MainNet, dt.default_path_specify(Coin::Bitcoin.id(), 0, 0, 0)
    )?;
    ...
    fn test_derive_change_internal_chain() -> Result<(), Error> {
        ...
        let derived = HDKey::new(
            ...
            HDNetworkType::MainNet, dt.default_path_specify(Coin::Bitcoin.id(), 0, 1, 0)
        )?;
        assert_eq!(
