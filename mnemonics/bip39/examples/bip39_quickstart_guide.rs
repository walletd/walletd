...
fn main() -> Result<(), walletd_bip39::Error> {
    ...
    println!("mnemonic phrase: {}", mnemonic.as_phrase());
    // can use the hex format specifier to print the seed as hex
    println!("mnemonic seed hex: {:x}", mnemonic.as_seed());
    // can use the as_bytes method to get the seed as a byte array
    println!("mnemonic seed as bytes: {:?}", mnemonic.as_seed().as_bytes());
    ...
    let mnemonic_1 = mnemonic_builder
        ...
        .build()?;
    println!("mnemonic_1 phrase: {}", mnemonic_1.as_phrase());
    println!("mnemonic_1 seed hex: {:x}", mnemonic_1.as_seed());
    ...
    println!("mnemonic_2 seed hex: {:x}", mnemonic_2.as_seed());
    println!(
    ...
    println!("mnemonic_3 seed hex: {:x}", mnemonic_3.as_seed());
    ...
    assert_eq!(format!("{:x}", restored_mnemonic_1.as_seed()), seed_hex_1);
    println!(
        "restored_mnemonic_1 phrase: {}",
        restored_mnemonic_1.as_phrase()
    );
    println!(
        "restored_mnemonic_1 seed hex: {:x}",
        restored_mnemonic_1.as_seed()
    );
    ...
    assert_eq!(format!("{:x}", restored_mnemonic_2.as_seed()), seed_hex_2);
    println!(
        "restored_mnemonic_2 phrase: {}",
        restored_mnemonic_2.as_phrase()
    );
    println!(
        "restored_mnemonic_2 seed hex: {:x}",
        restored_mnemonic_2.as_seed()
    );
