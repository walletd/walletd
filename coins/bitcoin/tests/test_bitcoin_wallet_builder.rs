    ...
    use std::str::FromStr;
    use walletd_bitcoin::{BitcoinWalletBuilder, AddressType, Error, Network, Seed, blockstream::Blockstream, HDKey};
    ...
    fn test_new() -> Result<(), Error> {
        ...
        assert_eq!(builder.blockchain_client().is_none(), default.blockchain_client().is_none());
        Ok(())
    }
    ...
    fn test_with_blockchain_client() -> Result<(), Error> {
        let mut builder = BitcoinWalletBuilder::default();
        builder.with_blockchain_client(Box::new(Blockstream::new("https://blockstream.info/testnet/api")?));
    ...
    fn test_with_hd_path_builder() -> Result<(), Error> {
        ...
        assert_eq!(builder.hd_path_builder().purpose.expect("checked is some"), builder.hd_purpose().expect("checked is some").to_shortform_num());
        ...
        assert_eq!(builder.hd_path_builder().coin_type.expect("checked is some"), 0);
        assert_eq!(builder.hd_path_builder().address_index.expect("checked is some"), 1);
