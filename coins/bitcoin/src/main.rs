...
use hex;
use walletd_bitcoin::{
    BTransaction, BitcoinWallet, Blockstream, Input, Output, Status, BLOCKSTREAM_URL,
};

fn test_compare_transaction() -> Result<(), anyhow::Error> {
    ...
    let unsigned_serialized = BTransaction::serialize(&transaction)?;
    ...
    let sighash_for_input1 = &transaction.txid()?;
    ...
    let raw_tx_hex = BTransaction::serialize(&signed_transaction)?;
...
fn main() {
    ...
    test_compare_transaction().unwrap();
}
