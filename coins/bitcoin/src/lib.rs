...
impl BitcoinWallet {
    ...
    pub fn get_block_count(client: &Client) -> Result<u64, String> {
        let block_count = client.get_block_count().unwrap();
        Ok(block_count)
    }
    pub fn get_best_block_hash(client: &Client) -> Result<BlockHash, String> {
        let best_block_hash = client.get_best_block_hash().unwrap();
        Ok(best_block_hash)
    }
    pub fn get_block_hash(client: &Client, height: u64) -> Result<BlockHash, String> {
    ...
    pub fn rpc_load_wallet(
        ...
        let loaded_wallet = client.load_wallet(wallet_path).unwrap();
        Ok(loaded_wallet)
    }
    pub fn rpc_list_wallets(client: &Client) -> Result<Vec<String>, String> {
        let listed_wallets = client.list_wallets().unwrap();
        Ok(listed_wallets)
    }
    ...
    pub fn rpc_unload_wallet(client: &Client, wallet_path: &str) -> Result<(), String> {
        let unloaded = client.unload_wallet(Some(wallet_path)).unwrap();
        Ok(unloaded)
    }
}
