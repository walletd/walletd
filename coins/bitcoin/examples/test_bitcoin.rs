...
async fn main() -> anyhow::Result<()> {
    ...
    let legacy = manager.get_receive_address("test-user", AddressType::Legacy).await?;
    println!("   Legacy (P2PKH): {}", legacy);
    
    let segwit_p2sh = manager.get_receive_address("test-user", AddressType::SegwitP2SH).await?;
    println!("   SegWit (P2SH): {}", segwit_p2sh);
    
    let native_segwit = manager.get_receive_address("test-user", AddressType::NativeSegwit).await?;
