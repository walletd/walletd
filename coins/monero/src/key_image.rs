use serde::Serialize;use anyhow::anyhow;
...
impl KeyDerivation {
    ...
    pub fn derive_private_key(
        ...
        MoneroPrivateKey::from_slice(&derived_scalar.to_bytes())
...
impl KeyImage {
    ...
    pub fn new(
        ...
        let derived_private_scalar = hash + Scalar::from_bytes_mod_order(private_spend_key.to_bytes());
        ...
        let ephemeral_private_key = MoneroPrivateKey::from_slice(&derived_private_scalar.to_bytes())
            .map_err(|_| Error::PrivateKeyError)?;
        let ephemeral_public_key = MoneroPublicKey::from_slice(&derived_public_point.compress().to_bytes())
            .map_err(|_| Error::PublicKeyError)?;
...
mod tests {
    ...
    fn test_key_image() {
        ...
        let expected_key_image = hex!("8a90c3e855fde0a85e71c9c345a26d094a56a5070b0bba6c1e9495bd49aa0741");
        let output_index = 1;
    ...
    fn test_generate_key_derivation() {
        let public_key_bytes = hex!("fdfd97d2ea9f1c25df773ff2c973d885653a3ee643157eb0ae2b6dd98f0b6984");
        let secret_key_bytes = hex!("eb2bd1cf0c5e074f9dbf38ebbc99c316f54e21803048c687a3bb359f7a713b02");
        let expected_key_deriv = hex!("4e0bd2c41325a1b89a9f7413d4d05e0a5a4936f241dccc3c7d0c539ffe00ef67");
        ...
        let public_key_bytes = hex!("1ebf8c3c296bb91708b09d9a8e0639ccfd72556976419c7dc7e6dfd7599218b9");
        let secret_key_bytes = hex!("e49f363fd5c8fc1f8645983647ca33d7ec9db2d255d94cd538a3cc83153c5f04");
        let expected_key_deriv = hex!("72903ec8f9919dfcec6efb5535490527b573b3d77f9890386d373c02bf368934");
        let public_key = MoneroPublicKey::from_slice(&public_key_bytes).unwrap();
        let secret_key = MoneroPrivateKey::from_slice(&secret_key_bytes).unwrap();
        let actual_key_deriv = KeyDerivation::generate(&public_key, &secret_key);
        assert_eq!(actual_key_deriv.to_bytes(), expected_key_deriv);
    ...
    fn test_derive_public_key() {
        ...
        let expected_derived_pub_key = hex!("d48008aff5f27d8fcdc2a3bf814ed3505530f598075f3bf7e868fea696b109f6");
        let derivation = KeyDerivation::from_slice(&key_deriv).unwrap();
        let base_key = MoneroPublicKey::from_slice(&base).unwrap();
        let actual_derived_pub_key = derivation.derive_public_key(output_index, &base_key).unwrap();
    ...
    fn test_derive_private_key() {
        ...
        let expected_derived_sec_key = hex!("55297d64b0c0556d5583ce0e30c2024ccce90c93d16bdeb4e40fce7afff87803");
        let derivation = KeyDerivation::from_slice(&key_deriv).unwrap();
        let base_key = MoneroPrivateKey::from_slice(&base).unwrap();
        let actual_derived_sec_key = derivation.derive_private_key(output_index, &base_key).unwrap();
