#!/bin/bash
echo "Merging canister implementations..."

# Check if we need to copy the original implementation
if [ -f "src/canister.rs.backup" ]; then
    # Extract the original CanisterClient implementation
    echo "Found backup file, extracting original implementation..."
    
    # Create a temporary merged file
    cat > src/canister/mod_merged.rs << 'INNER_EOF'
pub mod builder;
pub mod canisters;
pub mod testing;
...
pub trait WalletCanisterExt {
    ...
    async fn create_canister_client(&self, canister_id: &str) -> Result<CanisterClient, CanisterError>;
}

// Re-export commonly used types
pub use ic_agent::{Identity, Agent};
INNER_EOF
    
    # Replace the current mod.rs
    mv src/canister/mod_merged.rs src/canister/mod.rs
    echo "✅ Merged canister implementation"
fi
