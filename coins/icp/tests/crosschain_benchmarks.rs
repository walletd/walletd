use walletd_icp::crosschain::{
    BatchProcessor, ChainType, CrossChainCoordinator, CrossChainMessage, StateSynchronizer,
};
...
mod benchmarks {
    ...
    fn test_batch_processing_performance() {
        ...
        for i in 0..100 {
            let msg =
                CrossChainMessage::new(ChainType::ICP, ChainType::ETH, format!("message_{}", i));
    ...
    fn test_state_synchronization() {
        ...
        for i in 0..50 {
            let msg = CrossChainMessage::new(ChainType::ICP, ChainType::ETH, format!("sync_{}", i));
    ...
    fn test_cross_chain_routing() {
        ...
        for (i, (from, to)) in chains.iter().enumerate() {
            let msg = CrossChainMessage::new(from.clone(), to.clone(), format!("route_{}", i));
