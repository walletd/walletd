use walletd_icp::crosschain::{
    CrossChainCoordinator, AtomicSwap, SwapState,
    StateSynchronizer, BatchProcessor, CrossChainMessage, ChainType,
};
use std::sync::Arc;
    ...
    async fn test_multi_chain_coordination() {
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                let from = if i % 2 == 0 { ChainType::ICP } else { ChainType::ETH };
                let to = if i % 2 == 0 { ChainType::ETH } else { ChainType::ICP };
