...
async fn canister_call_examples() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let balance: TokenBalance = client.query_typed(
        "get_balance", 
    ...
    let result: Result<Nat, String> = client
        .update_typed("transfer", &transfer_args)
        .await?;
...
async fn error_handling_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let client = CanisterClient::local("invalid-canister-id").await
        .map_err(|e| {
    ...
    // Handle query errors
    match client.query_typed::<String>("non_existent_method", &()).await {
