...
async fn main() -> web3::Result<()> {
    ...
    let _block_number = eth_client.current_block_number().await;

    let _gas_price = eth_client.gas_price().await;
    ...
    // get_latest_block().await; println!("latest block {:#?}", latest_block);)
