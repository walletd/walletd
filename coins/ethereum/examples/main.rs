...
use web3::types::{Block, BlockId, BlockNumber, Transaction, TransactionId, H160, H256, U256, U64};
...
async fn main() -> web3::Result<()> {
    ...
    // let block_data = eth_client.web3.eth().block(BlockId::Number(BlockNumber::Latest)).await.unwrap().unwrap();
    ...
    // .unwrap()
    // .unwrap();
    // let latest_block: web3::types::block::Block<primitive_types::H256> = eth_client.get_latest_block().await.unwrap();
    //eth_client.get_latest_block().await;
