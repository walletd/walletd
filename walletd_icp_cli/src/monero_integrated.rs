            ...
            Ok(res) => {
                if let Ok(json) = res.json::<serde_json::Value>().await {
                    if let Some(result) = json.get("result") {
                        ...
                        let target = result.get("target_height").and_then(|t| t.as_u64()).unwrap_or(1);
                        let percent = (height as f64 / target as f64 * 100.0) as u32;
                        
                        return Ok(format!(
                            "⛏️ Mining Status:\n   Sync: {}% ({}/{})\n   {}",
                            percent, height, target,
                            if percent > 90 { "Mining active!" } else { "Mining will start at 90% sync" }
                        ));
                    }
                }
...
pub async fn show_integrated_menu(address: &str) -> Result<String, String> {
    ...
    println!("==============================");
    println!("Address: {}...{}", &address[..12], &address[address.len()-12..]);
