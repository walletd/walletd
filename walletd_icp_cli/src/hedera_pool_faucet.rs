        ...
        Some(account) => {
            ...
            if let Some(wallet) = &manager.hedera {
                if let Ok(actual_balance) = wallet.get_balance().await {
                    ...
                    let mut pool = ACCOUNT_POOL.write().await;
                    pool.report_balance(&account.account_id, actual_balance).await;
...
async fn add_account_to_pool() -> Result<(), String> {
    ...
    io::stdin().read_line(&mut account_id).map_err(|e| e.to_string())?;
    ...
    io::stdin().read_line(&mut balance_str).map_err(|e| e.to_string())?;
...
async fn save_and_reload(account_id: &str, private_key: &str) -> Result<(), String> {
    ...
    manager.init_hedera().await.map_err(|e| format!("Init failed: {}", e))?;
