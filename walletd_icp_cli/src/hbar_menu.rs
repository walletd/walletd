...
pub async fn handle_hbar_menu(
    ...
    loop {
        ...
        let (account_id, real_balance) = if let Some(wallet) = &manager.hedera {
            let account = wallet.account_id.as_ref()
                .unwrap_or(&"Not configured".to_string())
                .clone();
            
            // Get real balance
        println!("DEBUG: Calling get_balance for account {:?}", wallet.account_id);            let balance = match wallet.get_balance().await {
                Ok(bal) => bal,
                Err(_) => 0.0
            };
        ...
        match choice.trim() {
            ...
            "4" => send_hbar().await?,
            "E" | "e" => handle_exchange().await?,            "5" => view_history().await?,
            "W" | "w" => open_hedera_portal()?,            "R" | "r" => reload_wallet().await?,
...
async fn view_info() -> Result<(), String> {
    ...
    if let Some(wallet) = &manager.hedera {
        println!("\n=== Account Information ===");
        println!("Account: {}", wallet.account_id.as_ref().unwrap_or(&"Not set".to_string()));
    ...
    io::stdin().read_line(&mut String::new()).map_err(|e| e.to_string())?;
    Ok(())
}
...
async fn check_real_balance() -> Result<(), String> {
    ...
    if let Some(wallet) = &manager.hedera {
        ...
        println!("DEBUG: Calling get_balance for account {:?}", wallet.account_id);        match wallet.get_balance().await {
            Ok(balance) => {
...
async fn view_history() -> Result<(), String> {
    ...
    if let Some(wallet) = &manager.hedera {
        if let Some(account_id) = &wallet.account_id {
            ...
            println!("https://hashscan.io/testnet/account/{}/transactions", account_id);
        }
    }
