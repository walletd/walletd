...
/// and therefore the cryptographic strength of the wallet keys/addresses that can be derived from
/// it using the [`Seed`][Seed].
///
/// For example, a 13 word mnemonic phrase is essentially a friendly representation of a 128-bit key,
/// while a 25 word mnemonic phrase is essentially a 256-bit key.
///
/// If you know you want a specific phrase length, you can use the enum variant directly, for example
/// `MnemonicType::Words13`.
///
/// You can also get a `MnemonicType` that corresponds to one of the standard MONERO key sizes by
...
impl MnemonicType {
    ...
    ///
    /// Specifying a word count not provided for by the MONERO standard will return an `Error`
    ///
    /// # Example
    /// ```
    /// use walletd_monero_mnemonic::MnemonicType;
    ...
    ///
    /// Specifying a key size not provided for by the MONERO standard will return an `Error`
    ///
    /// # Example
    /// ```
    /// use walletd_monero_mnemonic::MnemonicType;
    ...
    ///
    /// Specifying a phrase that does not match one of the standard MONERO phrase lengths will return an `Error`
    ///
    /// # Example
    /// ```
    /// use walletd_monero_mnemonic::MnemonicType;
