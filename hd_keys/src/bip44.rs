...
pub fn derive_first_address(master_node: BIP32, coin: CryptoCoin) -> Result<BIP32, String> {
    let bip44_deriv_path = format!("{}{}{}", "m/44'/", coin.coin_type(), "'/0'/0/0");
    BIP32::derived_from_master_with_specified_path(
        &master_node,
        bip44_deriv_path)
}

// Later will need to add more functionality here to discover accounts and address that are being used and those that are not